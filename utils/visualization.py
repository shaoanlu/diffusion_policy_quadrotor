...
def visualize_quadrotor_simulation_result(quadrotor, states: np.ndarray, obs_center: np.ndarray, obs_radius: np.ndarray):
    def plot_obstacles(obs_center, obs_radius):
      for obs_p, obs_r in zip(obs_center, obs_radius):
        circle = plt.Circle(tuple(obs_p), obs_r, color='grey', fill=True, linestyle='--', linewidth=2, alpha=0.5)
